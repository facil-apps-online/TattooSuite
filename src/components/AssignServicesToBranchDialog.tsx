...
import { useAssignServiceToBranch, MasterService, useServiceBranchPrices } from "@/hooks/useServices";
import { useToast } from "@/hooks/use-toast";
import { Search } from "lucide-react";

interface AssignServicesToBranchDialogProps {
  isOpen: boolean;
...
const AssignServicesToBranchDialog: React.FC<AssignServicesToBranchDialogProps> = ({
  isOpen,
  onOpenChange,
  service,
  onSuccess,
  ...
  const { data: assignedBranchesData, isLoading: isLoadingAssignedBranches } = useServiceBranchPrices(service.id);
  ...
  const handleSubmit = () => {
    ...
    assignService(assignmentPayload, {
      onSuccess: () => {
        toast({ title: "Servicio Asignado", description: `El servicio '${service.name}' ha sido asignado a las sucursales seleccionadas.`, variant: "success" });
        onSuccess();
        onOpenChange(false);
      },
      onError: (error) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Asignar "{service.name}" a Sucursales</DialogTitle>
          <DialogDescription>
            Selecciona las sucursales a las que deseas asignar este servicio.
          </DialogDescription>
        </DialogHeader>
...
export default AssignServicesToBranchDialog;
