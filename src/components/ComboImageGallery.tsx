...
import { useComboImages, useDeleteComboImage, useSetPrimaryComboImage, useUploadComboImage } from "@/hooks/useComboImages"; // Use Combo Hooks
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Trash, Star, Upload, X } from "lucide-react";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from "@/components/ui/alert-dialog";
import { useToast } from "@/hooks/use-toast";
import { ImagePreviewDialog } from './ImagePreviewDialog'; // Re-use ImagePreviewDialog
import { ServiceImage } from './ServiceImage'; // Use ServiceImage for individual image display (will be renamed to ImageDisplay)
...
export const ComboImageGallery = ({ comboId }: ComboImageGalleryProps) => {
  const { data: images, isLoading, isError, refetch: refetchImages } = useComboImages(comboId); // Use Combo Hooks
  const { mutate: deleteImage } = useDeleteComboImage(); // Use Combo Hooks
  const { mutate: setPrimaryImage } = useSetPrimaryComboImage(); // Use Combo Hooks
  const { mutate: uploadImage, isPending: isUploading } = useUploadComboImage(); // Use Combo Hooks
    ...
    try {
      const uploadPromises = selectedFiles.map(async (file) => {
        ...
        return new Promise<void>((resolve, reject) => {
          uploadImage({ comboId, file: compressedFile }, { // Use comboId
            onSuccess: () => resolve(),
      ...
      <div className="space-y-6">
        ...
        {images && images.length > 0 && (
          <div className="space-y-2 pt-4 border-t">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {images.map((image: ComboImage) => ( // Use ComboImage
                <Card key={image.id} className="relative group overflow-hidden">
                  <CardContent className="p-0 cursor-pointer" onClick={() => handleOpenPreview(image.google_drive_file_id)}>
                    <ServiceImage imageUrl={image.google_drive_file_id} altText={`Imagen de combo ${image.id}`} className="object-cover w-full h-32" /> {/* Use ComboImage */}
                    {image.is_primary && (
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 flex items-center justify-center gap-2">
                      {!image.is_primary && (
                        <Button
                          ...
                          onClick={(e) => { e.stopPropagation(); setPrimaryImage({ comboId, imageId: image.id }); }} // Use comboId
                        >
                          <Star className="w-4 h-4" />
                        </Button>
                      )}
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => deleteImage({ imageId: image.id, comboId })}> {/* Use comboId */}
                                Eliminar
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
