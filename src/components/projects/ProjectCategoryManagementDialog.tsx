...
import { useProjectCategories, useDeleteProjectCategory, useToggleProjectCategoryStatus } from "@/hooks/useProjectCategories";
import { ProjectCategoryDialog } from "./ProjectCategoryDialog";

export function ProjectCategoryManagementDialog({ trigger }: { trigger: React.ReactNode }) {
  const [searchTerm, setSearchTerm] = useState("");
  const { data: categories, isLoading } = useProjectCategories();
  const toggleStatusMutation = useToggleProjectCategoryStatus();
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="sm:max-w-[700px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-primary">Gestión de Categorías de Proyecto</DialogTitle>
        </DialogHeader>
