...
import { ProductImage } from '../ProductImage'; // Assuming a generic reusable component
import { useProjectImages, useDeleteProjectImage, useSetPrimaryProjectImage, useUploadProjectImage } from "@/hooks/useProjects";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Trash, Star, Upload, X } from "lucide-react";
import { AlertDialog, AlertDialogAction, AlertDialogCancel, AlertDialogContent, AlertDialogDescription, AlertDialogFooter, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger } from "@/components/ui/alert-dialog";
import { useToast } from "@/hooks/use-toast";
import { ImagePreviewDialog } from '../ImagePreviewDialog'; // Assuming a generic reusable component
...
export const ProjectImageGallery = ({ projectId }: ProjectImageGalleryProps) => {
  ...
  const { mutate: uploadImage, isPending: isUploading } = useUploadProjectImage();
    ...
    try {
      const uploadPromises = selectedFiles.map(async (file) => {
        ...
        return new Promise<void>((resolve, reject) => {
          uploadImage({ projectId, file: compressedFile }, {
            onSuccess: () => resolve(),
              ...
              {images.map((image) => (
                <Card key={image.id} className="relative group overflow-hidden">
                  <CardContent className="p-0 cursor-pointer" onClick={() => handleOpenPreview(image.image_url)}>
                    <ProductImage imageUrl={image.image_url} altText={`Imagen de proyecto ${image.id}`} className="object-cover w-full h-32" />
                    {image.is_primary && (
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction onClick={() => deleteImage({ imageId: image.id, projectId })}>
                                Eliminar
                              </AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
