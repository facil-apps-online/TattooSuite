import { useProjectImages, useAssociateProjectImage, useDeleteProjectImage, useSetPrimaryProjectImage } from "@/hooks/useProjectImages";
...
export const ProjectImageGallery = ({ projectId }: ProjectImageGalleryProps) => {
  const { data: images, isLoading, isError, refetch: refetchImages } = useProjectImages(projectId);
  const { mutate: deleteImage, isDeleting } = useDeleteProjectImage();
  const { mutate: setPrimaryImage } = useSetPrimaryProjectImage();
  const { mutate: associateImage, isPending: isAssociating } = useAssociateProjectImage();
  ...
  const handleDeleteClick = (imageToDeleteId: string, projectId: string, google_drive_file_id: string) => {
    setDeletingImageId(imageToDeleteId);
    deleteImage(
      { imageId: imageToDeleteId, projectId, google_drive_file_id },
      {
        onSuccess: () => {
      ...
      <div className="space-y-6">
        <div className="space-y-4 p-4 border rounded-lg">
          <FileUploader
            ...
            path_components={['tenant', tenantId, 'Projects', projectId].filter(Boolean) as string[]}
            imageType="catalogo"
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {images.map((image) => (
                <Card key={image.id} className="relative group overflow-hidden">
                  ...
                  >
                    <ProjectImage imageUrl={image.google_drive_file_id} altText={`Imagen de proyecto ${image.id}`} className="object-cover w-full h-32" />
                    {image.is_primary && (
                    ...
                    <div className="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-50 transition-all duration-300 flex items-center justify-center gap-2">
                      {!image.is_primary && deletingImageId !== image.id && (
                        <Button
                          ...
                          onClick={(e) => { e.stopPropagation(); setPrimaryImage({ projectId, imageId: image.id }); }}
                        >
                          <Star className="w-4 h-4" />
                        </Button>
                      )}
                            ...
                            <AlertDialogFooter>
                              ...
                              <AlertDialogAction 
                                onClick={() => {
                                  ...
                                  handleDeleteClick(image.id, projectId, image.google_drive_file_id);
                                }}
                                disabled={deletingImageId === image.id}
                              >
