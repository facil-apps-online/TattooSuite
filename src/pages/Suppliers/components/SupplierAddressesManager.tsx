...
import { useSupplierAddresses, useCreateSupplierAddress, useUpdateSupplierAddress, useDeleteSupplierAddress } from "@/hooks/useSupplierRelations";
import type { SupplierAddress } from "@/hooks/useSupplierRelations";
...
const AddressFormCard = ({ supplierId, address, onSave, onCancel }: { supplierId: string, address?: SupplierAddress, onSave: () => void, onCancel: () => void }) => {
    const { mutate: createAddress } = useCreateSupplierAddress();
    const { mutate: updateAddress } = useUpdateSupplierAddress();
    ...
    const onSubmit = (values: AddressFormValues) => {
        if (address) {
            ...
            createAddress({ ...values, supplier_id: supplierId }, { onSuccess: onSave });
        }
    };
                ...
                <Form {...form}>
                    <form className="space-y-4">
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <Button type="button" onClick={(e) => { e.preventDefault(); form.handleSubmit(onSubmit)(); }}>Guardar</Button>
                        </div>
                    </form>
                </Form>
            </CardContent>
...
export const SupplierAddressesManager = ({ supplierId, isAdding, setIsAdding }: { supplierId: string, isAdding: boolean, setIsAdding: (isAdding: boolean) => void }) => {
    const { data: addresses, isLoading } = useSupplierAddresses(supplierId);
    const { mutate: deleteAddress } = useDeleteSupplierAddress();
                    ...
                    <Card key={address.id}>
                        <CardHeader className="flex flex-row items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <Button variant="ghost" size="icon" onClick={(e) => { e.preventDefault(); e.stopPropagation(); setEditingAddress(address); }}>
                                    <Edit className="w-4 h-4" />
                                </Button>
                                <Button variant="ghost" size="icon" onClick={() => deleteAddress({ id: address.id })}>
